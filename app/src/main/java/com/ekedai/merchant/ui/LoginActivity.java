...
import com.ekedai.merchant.models.login.LoginData;
...
public class LoginActivity extends AppCompatActivity {
                            ...
                            for (Store store : stores) {
                                FirebaseMessaging.getInstance().subscribeToTopic(store.id)
                                        ...
                                        .addOnFailureListener(e -> {
                                            stopLoading();
                                            removeUserData();
                                            handleError(-1, e.getLocalizedMessage(), "Failed to subscribe to store notifications");
                                            showFirebaseErrorNotification();
                                        });
                            }
                        } else {
    ...
    private void handleError(int responseCode, String debugString, String toastMessage) {
        Toast.makeText(this, toastMessage != null
                        ? toastMessage
                        : getString(R.string.request_failure_with_code, Integer.toString(responseCode)),
